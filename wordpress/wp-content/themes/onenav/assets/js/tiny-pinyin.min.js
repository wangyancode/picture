...
function ioGetPinyin(text) {
  if (!window.Pinyin || typeof Pinyin.convertToPinyin !== 'function') {
      return { full: '', first: '' };
  }

  const full = Pinyin.convertToPinyin(text, ' ', false).toLowerCase(); 
  const first = full.split(' ').map(w => w.charAt(0)).join('');
