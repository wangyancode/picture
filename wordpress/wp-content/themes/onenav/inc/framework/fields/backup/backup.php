 ...
 *
 */
if ( ! class_exists( 'IOCF_Field_backup' ) ) {
  class IOCF_Field_backup extends IOCF_Fields {
    ...
    public function render() {
      ...
      $unique = $this->unique;
      $nonce  = wp_create_nonce( 'iocf_backup_nonce' );
      $export = add_query_arg( array( 'action' => 'csf-export', 'unique' => $unique, 'nonce' => $nonce ), admin_url( 'admin-ajax.php' ) );

      echo $this->field_before();

      echo '<textarea name="iocf_import_data" class="csf-import-data"></textarea>';
      ...
      echo '<hr />';
      echo '<button type="submit" name="iocf_transient[reset]" value="reset" class="button csf-warning-primary csf-confirm csf-reset" data-unique="'. esc_attr( $unique ) .'" data-nonce="'. esc_attr( $nonce ) .'">'. esc_html__( 'Reset', 'csf' ) .'</button>';
