...
;(function( $, window, document, undefined ) {
  ...
  IOCF.helper = {
    ...
    name_nested_replace: function( $selector, field_id ) {
      ...
      var checks = [];
      var regex  = new RegExp(IOCF.helper.preg_quote(field_id +'[\\d+]'), 'g');
  ...
  // Expand All Options
  //
  $.fn.iocf_expand_all = function() {
    return this.each( function() {
      $(this).on('click', function( e ) {

        e.preventDefault();
        $('.csf-wrapper').toggleClass('csf-show-all');
        $('.csf-section').iocf_reload_script();
  ...
  //
  $.fn.iocf_page_templates = function() {
    if ( this.length ) {
        ...
        if ( value.length > 1 ) { // iowen.cn 3 to 1 兼容中文
          ...
          $titles.each( function() {
            ...
            if ( $title.text().match( new RegExp('.*?' + value + '.*?', 'i') ) ) {
              ...
              $field.parent().iocf_reload_script();
              /* iowen 添加父级目录 */
  ...
  $.fn.iocf_dependency = function() {
    return this.each( function() {
      ...
      if( $fields.length ) {
        ...
        var normal_ruleset = $.iocf_deps.createRuleset(),
            global_ruleset = $.iocf_deps.createRuleset(),
            group_ruleset  = $.iocf_deps.createRuleset(), // iowen.cn
            normal_depends = [],
            global_depends = [],
            group_depends  = []; // iowen.cn
        ...
        $fields.each( function() {
          ...
          var $field      = $(this),
              ...
              globals     = $field.data('depend-global') ? $field.data('depend-global').toString().split('|') : '',// iowen 兼容多个选项
              is_global   = $field.data('depend-global') ? true : false,
              ruleset     = ( is_global ) ? global_ruleset : normal_ruleset;
          var is_into = is_global;// iowen 兼容多个选项
          ...
          $.each(controllers, function( index, depend_id ) {
            ...
            if (globals[index] == 'group') { // iowen.cn
              group_depends.push(depend_id); // iowen.cn
            } else if (globals[index] == '') { // iowen.cn
              normal_depends.push(depend_id);// iowen.cn
            } else if (is_global) {
        ...
        if ( normal_depends.length ) {
          $.iocf_deps.enable($this, normal_ruleset, normal_depends);
        }

        // iowen.cn
        if (group_depends.length) {
          var $parent = $this.closest('.csf-cloneable-item');
          if($parent.length == 0){
            $parent = $this.closest('.group');
          }
          $.iocf_deps.enable($parent, global_ruleset, group_depends);
        }
        // iowen.cn END

        if ( global_depends.length ) {
          $.iocf_deps.enable(IOCF.vars.$body, global_ruleset, global_depends);
        }
  ...
  // Field: accordion
  //
  $.fn.iocf_field_accordion = function() {
    return this.each( function() {
    ...
    return this.each( function() {
      ...
      base.notificationOverlay = function() {
        ...
        if ( wp.customize.notifications && wp.customize.OverlayNotification ) {
          ...
          wp.customize.notifications.add( new wp.customize.OverlayNotification('iocf_field_backup_notification', {
            type: 'default',
            message: '&nbsp;',
            loading: true
          }));
      ...
      $reset.on('click', function( e ) {
        ...
        if ( IOCF.vars.is_confirm ) {
          ...
          .fail( function( response ) {
            ...
            wp.customize.notifications.remove('iocf_field_backup_notification');
          });
  ...
  // Field: background
  //
  $.fn.iocf_field_background = function() {
    return this.each( function() {
      $(this).find('.csf--background-image').iocf_reload_script();
    });
  };
  ...
  // Field: code_editor
  //
  $.fn.iocf_field_code_editor = function() {
    return this.each( function() {
      ...
      var $this       = $(this),
          ...
          data_editor = $textarea.data('config');
      ...
      var interval = setInterval(function () {
        if ( $this.is(':visible') ) {
          ...
          if ( data_editor.theme !== 'default' && IOCF.vars.code_themes.indexOf(data_editor.theme) === -1 ) {
            ...
            IOCF.vars.code_themes.push(data_editor.theme);
  ...
  // Field: datetime
  //
  $.fn.iocf_field_datetime = function() {
    return this.each( function() {
  ...
  // Field: fieldset
  //
  $.fn.iocf_field_fieldset = function() {
    return this.each( function() {
      $(this).find('.csf-fieldset-content').iocf_reload_script();
    });
  };
  ...
  // Field: gallery
  //
  $.fn.iocf_field_gallery = function() {
    return this.each( function() {
    ...
    return this.each( function() {
      ...
      $wrapper.accordion({
        ...
        activate: function( event, ui ) {
          ...
          if ( $panel.length && !$panel.data( 'opened' ) ) {
            ...
            $.each(inputs, function( key, $input ) {
              ...
              $input.on('change keyup csf.keyup', function() {
                ...
                $.each(inputs, function( key, $input ) {
                  ...
                  var input_type = $input.attr('type'); // iowen.cn
                  if (input_type === 'radio') {
                    input_value = $input.filter(':checked').val();
                  } else if (input_type === 'checkbox') {
                    input_value = $input.filter(':checked').map(function () {
                      return $(this).val();
                    }).get().join(title_prefix); // 将所有选中的值拼接成字符串
                  } // iowen.cn END
            ...
            $panel.iocf_reload_script();
            $panel.data( 'opened', true );
            $panel.data( 'retry', false );

          } else if ( $panel.data( 'retry' ) ) {

            $panel.iocf_reload_script_retry();
            $panel.data( 'retry', false );
      ...
      $wrapper.sortable({
        ...
        update: function( event, ui ) {
          ...
          IOCF.helper.name_nested_replace( $wrapper.children('.csf-cloneable-item'), field_id );
          $wrapper.iocf_customizer_refresh();
        ...
        // 如果父级包含 no-sort 类，则禁用 sortable
        disabled: $this.hasClass('no-sort')
      });
      ...
      $group.children('.csf-cloneable-add').on('click', function( e ) {
        ...
        var $cloned_item = $hidden.iocf_clone(true);
        ...
        $wrapper.iocf_customizer_refresh();
        $wrapper.iocf_customizer_listen({closest: true});
      ...
      var event_clone = function( e ) {
        ...
        var $this           = $(this),
            ...
            $cloned_helper  = $parent.children('.csf-cloneable-helper').iocf_clone(true),
            $cloned_title   = $parent.children('.csf-cloneable-title').iocf_clone(),
            $cloned_content = $parent.children('.csf-cloneable-content').iocf_clone(),
  ...
  $.fn.iocf_field_icon = function() {
    return this.each( function() {
      ...
      $this.on('click', '.csf-icon-add', function( e ) {
        ...
        IOCF.vars.$icon_target = $this;

        if ( !IOCF.vars.icon_modal_loaded ) {
          ...
          window.wp.ajax.post( 'csf-get-icons', {
            ...
            IOCF.vars.icon_modal_loaded = true;
            ...
            $load.on('click', 'i', function( e ) {
              ...
              IOCF.vars.$icon_target.find('.csf-icon-preview i').removeAttr('class').addClass(icon);
              IOCF.vars.$icon_target.find('.csf-icon-preview').removeClass('hidden');
              IOCF.vars.$icon_target.find('.csf-icon-remove').removeClass('hidden');
              IOCF.vars.$icon_target.find('input').val(icon).trigger('change');
  ...
  $.fn.iocf_field_link = function() {
    return this.each( function() {
      ...
      var $this   = $(this),
          ...
          uniqid  = IOCF.helper.uid('csf-wplink-textarea-');
  ...
  // Field: repeater
  //
  $.fn.iocf_field_repeater = function() {
    return this.each( function() {
      ...
      var $this     = $(this),
          ...
          field_id  = $wrapper.data('field-id'),
          is_number = Boolean( Number( $wrapper.data('title-number') ) ),// iowen 增加序号
          max       = parseInt( $wrapper.data('max') ),
          min       = parseInt( $wrapper.data('min') );

      $wrapper.children('.csf-repeater-item').children('.csf-repeater-content').iocf_reload_script();

      // iowen 增加序号
      var update_title_numbers = function( $selector ) {
        $selector.find('.csf-repeater-title-number').each( function( index ) {
          $(this).html( ( $(this).closest('.csf-repeater-item').index()+1 ) );
        });
      };
      // iowen 增加序号 END
      ...
      $wrapper.sortable({
        axis: 'y',
        handle: '.csf-repeater-helper',
        helper: 'original',
        cursor: 'move',
        placeholder: 'widget-placeholder',
        update: function( event, ui ) {

          IOCF.helper.name_nested_replace( $wrapper.children('.csf-repeater-item'), field_id );
          $wrapper.iocf_customizer_refresh();
          ui.item.iocf_reload_script_retry();

          if ( is_number ) {// iowen 增加序号
            update_title_numbers($wrapper);
          }
        },
        start: function(event, ui) {// iowen placeholder 高度
      ...
      $repeater.children('.csf-repeater-add').on('click', function( e ) {
        ...
        $wrapper.append($cloned_item);
        $cloned_item.children('.csf-repeater-content').iocf_reload_script();
        $wrapper.iocf_customizer_refresh();
        $wrapper.iocf_customizer_listen({closest: true});
      ...
      var event_clone = function( e ) {
        ...
        var $this           = $(this),
            ...
            $cloned_content = $parent.children('.csf-repeater-content').iocf_clone(),
            $cloned_helper  = $parent.children('.csf-repeater-helper').iocf_clone(true),
  ...
  // Field: sortable
  //
  $.fn.iocf_field_sortable = function() {
    return this.each( function() {
      ...
      $sortable.sortable({
        ...
        placeholder: 'widget-placeholder',
        update: function( event, ui ) {
          $sortable.iocf_customizer_refresh();
        }
      });

      $sortable.find('.csf-sortable-content').iocf_reload_script();
  ...
  // Field: spinner
  //
  $.fn.iocf_field_spinner = function() {
    return this.each( function() {
  ...
  // Field: switcher
  //
  $.fn.iocf_field_switcher = function() {
    return this.each( function() {
  ...
  $.fn.iocf_field_tabbed = function() {
    return this.each( function() {
      ...
      $contents.eq(0).iocf_reload_script();
  ...
  // Field: typography
  //
  $.fn.iocf_field_typography = function() {
    return this.each(function () {

      var base          = this;
      var $this         = $(this);
      var loaded_fonts  = [];
      var webfonts      = iocf_typography_json.webfonts;
      var googlestyles  = iocf_typography_json.googlestyles;
      var defaultstyles = iocf_typography_json.defaultstyles;
      ...
      base.init = function () {
        ...
        if ( $preview_block.length ) {
          ...
          $this.on('change', IOCF.helper.debounce( function( event ) {
  ...
  // Field: wp_editor
  //
  $.fn.iocf_field_wp_editor = function() {
    return this.each( function() {

      if ( typeof window.wp.editor === 'undefined' || typeof window.tinyMCEPreInit === 'undefined' || typeof window.tinyMCEPreInit.mceInit.iocf_wp_editor === 'undefined' ) {
        return;
      }
      ...
      var uid = IOCF.helper.uid('csf-editor-');

      $textarea.attr('id', uid);

      // Get default editor settings
      var default_editor_settings = {
        tinymce: window.tinyMCEPreInit.mceInit.iocf_wp_editor,
        quicktags: window.tinyMCEPreInit.qtInit.iocf_wp_editor
      };
      ...
      // Add Media buttons
      if ( field_editor_settings.media_buttons && window.iocf_media_buttons ) {
        ...
        if ( $editor_buttons.length ) {
          ...
          var $media_buttons = $(window.iocf_media_buttons);
  ...
  // Confirm
  //
  $.fn.iocf_confirm = function() {
    return this.each( function() {
      $(this).on('click', function( e ) {

        var confirm_text   = $(this).data('confirm') || window.iocf_vars.i18n.confirm;
        var confirm_answer = confirm( confirm_text );

        if ( confirm_answer ) {
          IOCF.vars.is_confirm = true;
          IOCF.vars.form_modified = false;
        } else {
          e.preventDefault();
          return false;
        }
      ...
      $this.on('click', function( e ) {
        ...
        if ( !flooding ) {
          ...
          if ( $this.hasClass('csf-save-ajax') ) {
            ...
            window.wp.ajax.post( 'iocf_'+ $panel.data('unique') +'_ajax_save', {
              data: $('#csf-form').serializeJSONIOCF()
            })
            .done( function( response ) {
  ...
  $.fn.iocf_options = function() {
    return this.each( function() {
      ...
      IOCF.vars.$form_warning = $form_warning;

      // Shows a message white leaving theme options without saving
      if ( $form_warning.length ) {

        window.onbeforeunload = function() {
          return ( IOCF.vars.form_modified ) ? true : undefined;
        };

        $content.on('change keypress', ':input:not(.not-change)', function() {  // iowen.cn     ADD :not(.not-change)
          if ( !IOCF.vars.form_modified ) {
            $form_success.hide();
            $form_warning.fadeIn('fast');
            IOCF.vars.form_modified = true;
          }
        });
    ...
    return this.each( function() {
      ...
      if ( $form.attr('id') === 'addtag' ) {
        ...
        var $submit = $form.find('#submit'),
            $cloned = $this.children('.csf-field').iocf_clone();
        ...
        $submit.on( 'click', function() {
          ...
          if ( !$form.find('.form-required').hasClass('form-invalid') ) {
            ...
            $cloned = $cloned.iocf_clone();

            $this.iocf_reload_script();
  ...
  $.fn.iocf_shortcode = function() {
    ...
    return this.each( function() {
      ...
      $select.on( 'change', function() {
        ...
        if ( sc_key ) {
          ...
          .done( function( response ) {
            ...
            $cloned = $appended.find('.csf--repeat-shortcode').iocf_clone();

            $appended.iocf_reload_script();
            $appended.find('.csf-fields').iocf_reload_script();
      ...
      $insert.on('click', function( e ) {
        ...
        var shortcode = '';
        var serialize = $modal.find('.csf-field:not(.csf-depend-on)').find(':input:not(.ignore)').serializeObjectIOCF();
        ...
        if ( gutenberg_id ) {
          ...
          var content = window.iocf_gutenberg_props.attributes.hasOwnProperty('shortcode') ? window.iocf_gutenberg_props.attributes.shortcode : '';
          window.iocf_gutenberg_props.setAttributes({shortcode: content + shortcode});
      ...
      $modal.on('click', '.csf--repeat-button', function( e ) {
        ...
        var $new_clone  = $cloned.iocf_clone();
        var $remove_btn = $new_clone.find('.csf-repeat-remove');

        var $appended = $new_clone.appendTo( $repeatable );

        $new_clone.find('.csf-fields').iocf_reload_script();

        IOCF.helper.name_nested_replace( $modal.find('.csf--repeat-shortcode'), sc_group );
  ...
  IOCF.funcs.parse_color = function( color ) {
  ...
  $.fn.iocf_color = function() {
    return this.each( function() {

      var $input        = $(this),
          picker_color  = IOCF.funcs.parse_color( $input.val() ),
          palette_color = window.iocf_vars.color_palette.length ? window.iocf_vars.color_palette : true,
          $container;
          ...
          $transparent_slider.slider({
            ...
            create: function() {
              ...
              $container.on('click', '.wp-picker-default', function() {
                ...
                var default_color = IOCF.funcs.parse_color( $input.data('default-color') ),
    ...
    return this.each( function() {
      ...
      var $this       = $(this),
          ...
          set_options = $.extend({
            ...
            width: set_width,
            no_results_text: window.iocf_vars.i18n.no_results_text,
      ...
      if ( is_ajax ) {
        ...
        var set_ajax_options = $.extend({
          ...
          min_length: 2,
          type_delay: 500,
          typing_text: window.iocf_vars.i18n.typing_text,
          searching_text: window.iocf_vars.i18n.searching_text,
          no_results_text: window.iocf_vars.i18n.no_results_text,
        }, $this.data('chosen-settings'));

        $this.IOCFAjaxChosen(set_ajax_options);
      ...
      if ( is_multiple ) {
        ...
        $this.IOCFChosenOrder($hidden_value, true);
    ...
    return this.each( function() {
      ...
      $this.on({
        mouseenter: function() {
          ...
          offset_left = ( IOCF.vars.is_rtl ) ? ( $this.offset().left + 24 ) : ( $this.offset().left - $tooltip.outerWidth() );
  ...
  // Customize Refresh
  //
  $.fn.iocf_customizer_refresh = function() {
    return this.each( function() {
  ...
  //
  $.fn.iocf_customizer_listen = function( options ) {
    ...
    return this.each( function() {
      ...
      $input.on('change keyup csf.change', function() {
        ...
        var obj = $this.find(':input').serializeObjectIOCF();
  ...
  $(document).on('expanded', '.control-section', function() {
    ...
    if ( $this.hasClass('open') && !$this.data('inited') ) {
      ...
      if ( $fields.length ) {
        $this.iocf_dependency();
        $fields.iocf_reload_script({dependency: false});
        $complex.iocf_customizer_listen();
      }
  ...
  // Window on resize
  //
  IOCF.vars.$window.on('resize csf.resize', IOCF.helper.debounce( function( event ) {

    var window_width = navigator.userAgent.indexOf('AppleWebKit/') > -1 ? IOCF.vars.$window.width() : window.innerWidth;

    if ( window_width <= 782 && !IOCF.vars.onloaded ) {
      $('.csf-section').iocf_reload_script();
      IOCF.vars.onloaded  = true;
    }
  ...
  $.fn.iocf_widgets = function() {
    return this.each( function() {
      ...
      $('.widgets-sortables, .control-section-sidebar').on('sortstop', function( event, ui ) {
        ui.item.find('.csf-fields').iocf_reload_script_retry();
      });
  ...
  //
  $.fn.iocf_nav_menu = function() {
    return this.each( function() {

      var $navmenu = $(this);

      $navmenu.on('click', 'a.item-edit', function() {
        $(this).closest('li.menu-item').find('.csf-fields').iocf_reload_script();
      });
  ...
  // Retry Plugins
  //
  $.fn.iocf_reload_script_retry = function() {
    return this.each( function() {

      var $this = $(this);

      if ( $this.data('inited') ) {
        $this.children('.csf-field-wp_editor').iocf_field_wp_editor();
      }
  ...
  // Reload Plugins
  //
  $.fn.iocf_reload_script = function( options ) {
    ...
    return this.each( function() {
      ...
      if ( !$this.data('inited') ) {
        ...
        // Field plugins
        $this.children('.csf-field-accordion').iocf_field_accordion();
        $this.children('.csf-field-backup').iocf_field_backup();
        $this.children('.csf-field-background').iocf_field_background();
        $this.children('.csf-field-code_editor').iocf_field_code_editor();
        $this.children('.csf-field-date').iocf_field_date();
        $this.children('.csf-field-datetime').iocf_field_datetime();
        $this.children('.csf-field-fieldset').iocf_field_fieldset();
        $this.children('.csf-field-gallery').iocf_field_gallery();
        $this.children('.csf-field-group').iocf_field_group();
        $this.children('.csf-field-icon').iocf_field_icon();
        $this.children('.csf-field-link').iocf_field_link();
        $this.children('.csf-field-media').iocf_field_media();
        $this.children('.csf-field-map').iocf_field_map();
        $this.children('.csf-field-repeater').iocf_field_repeater();
        $this.children('.csf-field-slider').iocf_field_slider();
        $this.children('.csf-field-sortable').iocf_field_sortable();
        $this.children('.csf-field-sorter').iocf_field_sorter();
        $this.children('.csf-field-spinner').iocf_field_spinner();
        $this.children('.csf-field-switcher').iocf_field_switcher();
        $this.children('.csf-field-tabbed').iocf_field_tabbed();
        $this.children('.csf-field-typography').iocf_field_typography();
        $this.children('.csf-field-upload').iocf_field_upload();
        $this.children('.csf-field-wp_editor').iocf_field_wp_editor();

        // Field colors
        $this.children('.csf-field-border').find('.csf-color').iocf_color();
        $this.children('.csf-field-background').find('.csf-color').iocf_color();
        $this.children('.csf-field-color').find('.csf-color').iocf_color();
        $this.children('.csf-field-color_group').find('.csf-color').iocf_color();
        $this.children('.csf-field-link_color').find('.csf-color').iocf_color();
        $this.children('.csf-field-typography').find('.csf-color').iocf_color();

        // Field chosenjs
        $this.children('.csf-field-select').find('.csf-chosen').iocf_chosen();

        // Field Checkbox
        $this.children('.csf-field-checkbox').find('.csf-checkbox').iocf_checkbox();
        $this.children('.csf-field-checkbox').find('.csf-checkbox-all').iocf_checkbox_all();

        // Field Siblings
        $this.children('.csf-field-button_set').find('.csf-siblings').iocf_siblings();
        $this.children('.csf-field-image_select').find('.csf-siblings').iocf_siblings();
        $this.children('.csf-field-palette').find('.csf-siblings').iocf_siblings();

        // Help Tooptip
        $this.children('.csf-field').find('.csf-help').iocf_help();
  ...
  $(document).ready( function() {
    ...
    $('.csf-save').iocf_save();
    $('.csf-options').iocf_options();
    $('.csf-sticky-header').iocf_sticky();
    $('.csf-nav-options').iocf_nav_options();
    $('.csf-nav-metabox').iocf_nav_metabox();
    $('.csf-taxonomy').iocf_taxonomy();
    $('.csf-page-templates').iocf_page_templates();
    $('.csf-post-formats').iocf_post_formats();
    $('.csf-shortcode').iocf_shortcode();
    $('.csf-search').iocf_search();
    $('.csf-confirm').iocf_confirm();
    $('.csf-expand-all').iocf_expand_all();
    $('.csf-onload').iocf_reload_script();
    $('#widgets-editor').iocf_widgets();
    $('#widgets-right').iocf_widgets();
    $('#menu-to-edit').iocf_nav_menu();
