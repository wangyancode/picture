 ...
 *
 */
if ( ! class_exists( 'IOCF_Nav_Menu_Options' ) ) {
  class IOCF_Nav_Menu_Options extends IOCF_Abstract{
    ...
    public function wp_edit_nav_menu_walker( $class, $menu_id ) {
      ...
      if( version_compare( $wp_version, '5.4.0', '<' ) ) {
        ...
        if ( ! class_exists( 'IOCF_Walker_Nav_Menu_Edit' ) ) {
          IOCF::include_plugin_file( 'functions/walker.php' );
        }

        return 'IOCF_Walker_Nav_Menu_Edit';
    ...
    public function wp_nav_menu_item_custom_fields( $menu_item_id, $item, $depth, $args ) {
      ...
      $errors = ( ! empty( $menu_item_id ) ) ? get_post_meta( $menu_item_id, '_iocf_errors_'. $this->unique, true ) : array();
      $errors = ( ! empty( $errors ) ) ? $errors : array();
      $class  = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_post_meta( $menu_item_id, '_iocf_errors_'. $this->unique );
      }
        ...
        foreach ( $this->sections as $section ) {
          ...
          if ( ! empty( $section['fields'] ) ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              IOCF::field( $field, $this->get_meta_value( $menu_item_id, $field ), $this->unique .'['. $menu_item_id .']', 'menu' );
    ...
    public function wp_update_nav_menu_item( $menu_id, $menu_item_db_id, $menu_item_args ) {
      ...
      $data = apply_filters( "iocf_{$this->unique}_save", $data, $menu_item_db_id, $this );

      do_action( "iocf_{$this->unique}_save_before", $data, $menu_item_db_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $menu_item_db_id, '_iocf_errors_'. $this->unique, $errors );
        }

      }

      do_action( "iocf_{$this->unique}_saved", $data, $menu_item_db_id, $this );

      do_action( "iocf_{$this->unique}_save_after", $data, $menu_item_db_id, $this );
