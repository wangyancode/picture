 ...
 *
 */
if ( ! class_exists( 'IOCF_Options' ) ) {
  class IOCF_Options extends IOCF_Abstract {
    ...
    public function __construct( $key, $params = array() ) {
      ...
      $this->unique   = $key;
      $this->args     = apply_filters( "iocf_{$this->unique}_args", wp_parse_args( $params['args'], $this->args ), $this );
      $this->sections = apply_filters( "iocf_{$this->unique}_sections", $params['sections'], $this );
      ...
      add_action( 'wp_ajax_iocf_'. $this->unique .'_ajax_save', array( $this, 'ajax_save' ) );
    ...
    public function set_options( $ajax = false ) {
      ...
      // Set variables.
      $data      = array();
      $noncekey  = 'iocf_options_nonce'. $this->unique;
      $nonce     = ( ! empty( $response[$noncekey] ) ) ? $response[$noncekey] : '';
      $options   = ( ! empty( $response[$this->unique] ) ) ? $response[$this->unique] : array();
      $transient = ( ! empty( $response['iocf_transient'] ) ) ? $response['iocf_transient'] : array();

      if ( wp_verify_nonce( $nonce, 'iocf_options_nonce' ) ) {

        $importing  = false;
        $section_id = ( ! empty( $transient['section'] ) ) ? $transient['section'] : '';

        if ( ! $ajax && ! empty( $response[ 'iocf_import_data' ] ) ) {

          // XSS ok.
          // No worries, This "POST" requests is sanitizing in the below foreach. see #L337 - #L341
          $import_data  = json_decode( wp_unslash( trim( $response[ 'iocf_import_data' ] ) ), true );
        ...
        if ( ! empty( $transient['reset'] ) ) {
          ...
          //添加挂钩 iowen.cn
          do_action("iocf_{$this->unique}_reset_before", $data, $this);
          ...
          //添加挂钩 iowen.cn
          do_action("iocf_{$this->unique}_reset_section_before", $data, $this);
        } else {
        ...
        $data = apply_filters( "iocf_{$this->unique}_save", $data, $this );

        do_action( "iocf_{$this->unique}_save_before", $data, $this );

        $this->options = $data;

        $this->save_options( $data );

        do_action( "iocf_{$this->unique}_save_after", $data, $this );
    ...
    public function save_options( $data ) {
      ...
      do_action( "iocf_{$this->unique}_saved", $data, $this );
    ...
    public function add_options_html() {
      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<input type="hidden" class="csf-section-id" name="iocf_transient[section]" value="1">';

        wp_nonce_field( 'iocf_options_nonce', 'iocf_options_nonce'. $this->unique );
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="iocf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'csf' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'csf' ) .'">' : '';
            echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="iocf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'csf' ) : esc_html__( 'Reset', 'csf' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'csf' ) .'">' : '';
            echo '</div>';
            ...
            foreach ( $this->pre_sections as $section ) {
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  IOCF::field( $field, $value, $this->unique, 'options' );
        ...
        if ( ! empty( $this->args['show_footer'] ) ) {
          ...
          echo '<div class="csf-buttons">';
          echo '<input type="submit" name="iocf_transient[save]" class="button button-primary csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'csf' ) .'" data-save="'. esc_html__( 'Saving...', 'csf' ) .'">';
          echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="iocf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'csf' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'csf' ) .'">' : '';
          echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="iocf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'csf' ) : esc_html__( 'Reset', 'csf' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'csf' ) .'">' : '';
          echo '</div>';
