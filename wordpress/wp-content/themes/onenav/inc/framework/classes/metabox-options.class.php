 ...
 *
 */
if ( ! class_exists( 'IOCF_Metabox' ) ) {
  class IOCF_Metabox extends IOCF_Abstract{
    ...
    public function add_meta_box_content( $post, $callback ) {
      ...
      $errors    = ( is_object ( $post ) ) ? get_post_meta( $post->ID, '_iocf_errors_'. $this->unique, true ) : array();
      $errors    = ( ! empty( $errors ) ) ? $errors : array();
      $theme     = ( $this->args['theme'] ) ? ' csf-theme-'. $this->args['theme'] : '';
      $nav_type  = ( $this->args['nav'] === 'inline' ) ? 'inline' : 'normal';

      if ( is_object ( $post ) && ! empty( $errors ) ) {
        delete_post_meta( $post->ID, '_iocf_errors_'. $this->unique );
      }

      wp_nonce_field( 'iocf_metabox_nonce', 'iocf_metabox_nonce'. $this->unique );
            ...
            foreach ( $this->sections as $section ) {
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  IOCF::field( $field, $this->get_meta_value( $field ), $this->unique, 'metabox' );
    ...
    public function save_meta_box( $post_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'iocf_metabox_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'iocf_metabox_nonce' ) ) {
        return $post_id;
      }
      ...
      $data = apply_filters( "iocf_{$this->unique}_save", $data, $post_id, $this );

      do_action( "iocf_{$this->unique}_save_before", $data, $post_id, $this );
      ...
      if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_post_meta( $post_id, '_iocf_errors_'. $this->unique, $errors );
        }

      }

      do_action( "iocf_{$this->unique}_saved", $data, $post_id, $this );

      do_action( "iocf_{$this->unique}_save_after", $data, $post_id, $this );
