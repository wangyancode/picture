...
if ( ! class_exists( 'IOCF_Setup' ) ) {
  class IOCF_Setup {
    ...
    public function __construct() {
      ...
      add_action( 'after_setup_theme', array( 'IOCF', 'setup' ) );
      add_action( 'init', array( 'IOCF', 'setup' ) );
      add_action( 'switch_theme', array( 'IOCF', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'IOCF', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'IOCF', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'IOCF', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'IOCF', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'IOCF_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            IOCF_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'IOCF_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
        foreach ( self::$args['customize_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            IOCF_Customize_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'IOCF_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
        foreach ( self::$args['metabox_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            IOCF_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'IOCF_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
        foreach ( self::$args['nav_menu_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            IOCF_Nav_Menu_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'IOCF_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
        foreach ( self::$args['profile_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            IOCF_Profile_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'IOCF_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
        ...
        foreach ( self::$args['taxonomy_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            IOCF_Taxonomy_Options::instance( $key, $params );
      ...
      if ( class_exists( 'IOCF_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
        ...
        foreach ( self::$args['widget_options'] as $key => $value ) {
          if ( ! isset( self::$inited[$key] ) ) {
            ...
            self::$inited[$key] = true;
            $wp_widget_factory->register( IOCF_Widget::instance( $key, $value ) );
      ...
      $params = array();
      if ( class_exists( 'IOCF_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
        foreach ( self::$args['comment_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            IOCF_Comment_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'IOCF_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
        foreach ( self::$args['shortcode_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            IOCF_Shortcoder::instance( $key, $params );

          }
        }

        // Once editor setup for gutenberg and media buttons
        if ( class_exists( 'IOCF_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
          foreach ( self::$shortcode_instances as $instance ) {
            if ( ! empty( $instance['show_in_editor'] ) ) {
              IOCF_Shortcoder::once_editor_setup();
              break;
            }
          }
        }
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      $path     = '';
      $file     = ltrim( $file, '/' );
      $override = apply_filters( 'iocf_override', 'csf-override' );
    ...
    public static function includes() {
      ...
      $fields = apply_filters( 'iocf_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'IOCF_Field_'. $field ) && class_exists( 'IOCF_Fields' ) ) {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( ! apply_filters( 'iocf_enqueue_assets', self::$enqueue ) ) {
        return;
      }
      ...
      if ( apply_filters( 'iocf_fa4', false ) ) {
        wp_enqueue_style( 'csf-fa', 'https://cdn.bootcdn.net/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css', array(), null, 'all' );
      } else {
        wp_enqueue_style( 'csf-fa5', 'https://cdn.bootcdn.net/ajax/libs/font-awesome/5.15.4/css/all.min.css', array(), null, 'all' );
        wp_enqueue_style( 'csf-fa5-v4-shims', 'https://cdn.bootcdn.net/ajax/libs/font-awesome/5.15.4/css/v4-shims.min.css', array(), null, 'all' );
      }
      ...
      // Main variables
      wp_localize_script( 'csf', 'iocf_vars', array(
        'color_palette'     => apply_filters( 'iocf_color_palette', array() ),
        'i18n'              => array(
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'IOCF_Field_' . $field['type'];
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      if ( ! empty( $field_type ) ) {
        ...
        $classname = 'IOCF_Field_'. $field_type;
