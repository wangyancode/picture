 ...
 *
 */
if ( ! class_exists( 'IOCF_Comment_Metabox' ) ) {
  class IOCF_Comment_Metabox extends IOCF_Abstract{
    ...
    public function add_comment_meta_box_content( $comment, $callback ) {
      ...
      $errors   = ( is_object ( $comment ) ) ? get_comment_meta( $comment->comment_ID, '_iocf_errors_'. $this->unique, true ) : array();
      $errors   = ( ! empty( $errors ) ) ? $errors : array();
      $theme    = ( $this->args['theme'] ) ? ' csf-theme-'. $this->args['theme'] : '';
      $nav_type = ( $this->args['nav'] === 'inline' ) ? 'inline' : 'normal';

      if ( is_object( $comment ) && ! empty( $errors ) ) {
        delete_comment_meta( $comment->comment_ID, '_iocf_errors_'. $this->unique );
      }

      wp_nonce_field( 'iocf_comment_metabox_nonce', 'iocf_comment_metabox_nonce'. $this->unique );
            ...
            foreach ( $this->sections as $section ) {
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  IOCF::field( $field, $this->get_meta_value( $comment->comment_ID, $field ), $this->unique, 'comment_metabox' );
    ...
    public function save_comment_meta_box( $comment_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'iocf_comment_metabox_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'iocf_comment_metabox_nonce' ) ) {
        return $comment_id;
      }
      ...
      $data = apply_filters( "iocf_{$this->unique}_save", $data, $comment_id, $this );

      do_action( "iocf_{$this->unique}_save_before", $data, $comment_id, $this );
      ...
      if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_comment_meta( $comment_id, '_iocf_errors_'. $this->unique, $errors );
        }

      }

      do_action( "iocf_{$this->unique}_saved", $data, $comment_id, $this );

      do_action( "iocf_{$this->unique}_save_after", $data, $comment_id, $this );
