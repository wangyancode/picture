 ...
 *
 */
if ( ! class_exists( 'IOCF_Shortcoder' ) ) {
  class IOCF_Shortcoder extends IOCF_Abstract{
    ...
    public $args         = array(
      ...
      'class'            => '',
      'gutenberg'        => array(
        'title'          => 'IOCF Shortcodes',
        'description'    => 'IOCF Shortcode Block',
        'icon'           => 'screenoptions',
        'category'       => 'widgets',
    ...
    public function __construct( $key, $params = array() ) {
      ...
      if ( ! empty( $this->args['show_in_editor'] ) ) {
        ...
        IOCF::$shortcode_instances[] = wp_parse_args( array( 'name' => 'csf/'. $name, 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if ( IOCF::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'IOCF', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'IOCF_Field_icon', 'add_footer_modal_icon' ) );
    ...
    ?>
      <div id="csf-modal-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui csf-modal csf-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-modal-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'iocf_shortcode_nonce' ) ); ?>">
    ...
    public function get_shortcode() {
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'iocf_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              IOCF::field( $field, $field_default, $shortcode, 'shortcode' );
            ...
            echo '<div class="csf--repeatable">';
              ...
              echo '<div class="csf--repeat-shortcode">';
                ...
                foreach ( $repeatable_fields as $field ) {
                  ...
                  IOCF::field( $field, $field_default, $inner_shortcode.'[0]', 'shortcode' );
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'IOCF_Shortcoder', 'add_guteberg_blocks' ) );
      }

      if ( iocf_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'IOCF_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      wp_enqueue_script( 'csf-gutenberg-block', IOCF::include_plugin_url( 'assets/js/gutenberg.js' ), $depends );

      wp_localize_script( 'csf-gutenberg-block', 'iocf_gutenberg_blocks', IOCF::$shortcode_instances );

      foreach ( IOCF::$shortcode_instances as $block ) {
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( IOCF::$shortcode_instances as $value ) {
