 ...
 *
 */
if ( ! class_exists( 'IOCF_Taxonomy_Options' ) ) {
  class IOCF_Taxonomy_Options extends IOCF_Abstract{
    ...
    public function render_taxonomy_form_fields( $term ) {
      ...
      $errors    = ( ! empty( $term_id ) ) ? get_term_meta( $term_id, '_iocf_errors_'. $this->unique, true ) : array();
      $errors    = ( ! empty( $errors ) ) ? $errors : array();
      $class     = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_term_meta( $term_id, '_iocf_errors_'. $this->unique );
      }

      wp_nonce_field( 'iocf_taxonomy_nonce', 'iocf_taxonomy_nonce'. $this->unique );
        ...
        if ( $taxonomy === $this->taxonomy ) {
          ...
          if ( ! empty( $section['fields'] ) ) {
            foreach ( $section['fields'] as $field ) {
              ...
              IOCF::field( $field, $this->get_meta_value( $field, $term_id ), $this->unique, 'taxonomy' );
    ...
    public function save_taxonomy( $term_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'iocf_taxonomy_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';
      $taxonomy = ( ! empty( $_POST[ 'taxonomy' ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ 'taxonomy' ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'iocf_taxonomy_nonce' ) ) {
        return $term_id;
      }
      ...
      $data = apply_filters( "iocf_{$this->unique}_save", $data, $term_id, $this );

      do_action( "iocf_{$this->unique}_save_before", $data, $term_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_term_meta( $term_id, '_iocf_errors_'. $this->unique, $errors );
        }

      }

      do_action( "iocf_{$this->unique}_saved", $data, $term_id, $this );

      do_action( "iocf_{$this->unique}_save_after", $data, $term_id, $this );
